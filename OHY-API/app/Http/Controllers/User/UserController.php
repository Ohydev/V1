            ...
            try {
                ...
                try {
                    if ($existingUser && !$existingUser->is_email_verification_complete) {
                        ...
                        $updateCondition = array('user_id' => $existingUser->user_id);
    ...
    public function userLogin(Request $request)
    {
        ...
        if (!$validation->fails()) {
            try {
                ...
                // Prepare query condition to find user by email
                $queryCondition = array('email' => $data['email']); // Query condition for email lookup
            ...
            try {
                ...
                if (!empty($data['state']) && !empty($data['zipcode'])) {
                    ...
                    try {
                        ...
                        Log::warning('Zippopotam lookup failed in updateUserProfile', array(
                            'zipcode' => $zipcode,
                            'error' => $e->getMessage(),
                        ));
                    ...
                    if ($response->failed()) {
                        Log::warning('Zippopotam returned non-success status in updateUserProfile', array(
                            'zipcode' => $zipcode,
                ...
                $queryCondition = array('user_id' => $user->user_id);
                ...
                try {
                    ...
                    $hostLookupCondition = array('email' => $user->email);
        ...
        if (!$validation->fails()) {
            try {
                ...
                try {
                    ...
                    $hostLookupCondition = array('email' => $user->email);
                    if ($hostUserModel->check_host_user_exists($hostLookupCondition)) {
                        $hostUserModel->update_host_user_data($hostLookupCondition, array(
        ...
        if (!$validation->fails()) {
            try {
                ...
                try {
                    ...
                    Log::warning('Zippopotam lookup failed', array(
                        'zipcode' => $zipcode,
                        'error' => $e->getMessage(),
                    ));
                ...
                if ($response->failed()) {
                    Log::warning('Zippopotam returned non-success status', array(
                        'zipcode' => $zipcode,
                        'status' => $response->status(),
                        'body' => $response->body(),
                    ));
                ...
                if (!$existingUser || ($existingUser && !$existingUser->is_email_verification_complete)) {
                    ...
                    try {
                        if ($existingUser && !$existingUser->is_email_verification_complete) {
                            ...
                            $updateCondition = array('user_id' => $existingUser->user_id);
                            $userModel->update_user_data($updateCondition, array(
