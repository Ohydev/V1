...
use App\Models\OrderModel as UserOrderModel;
...
class SuperAdminController extends Controller
{
    ...
    public function getSuperAdminProfile(Request $request)
    {
        ...
        $result = array(); // Holds final success/error payload
    ...
    public function superAdminLogin(Request $request)
    {
        ...
        $result = array(); // Result array will hold final response structure
    ...
    public function updateSuperAdminProfile(Request $request)
    {
        ...
        $result = array(); // Will hold success/error structure
    ...
    public function getSuperAdminDashboard(Request $request)
    {
        ...
        $result = array(); // Stores success/error payload
        ...
        try {
            ...
            $recentEventsArray = array(); // Holds formatted recent events
        ...
        try {
            ...
            foreach ($allMonths as $month) {
                ...
                if ($isUpcomingMonth) {
                    ...
                    $eventsData[] = array($monthAbbr => null);
                    $revenueData[] = array($monthAbbr => null);
                    $profitData[] = array($monthAbbr => null);
                    continue;
                }
                ...
                $monthAbbr = $monthAbbreviations[$month];
                $eventsData[] = array($monthAbbr => (int)$eventsCount);
                $revenueData[] = array($monthAbbr => round($revenue, 2));
                $profitData[] = array($monthAbbr => $profit);
            }
    ...
    public function getSuperAdminReportsList(Request $request)
    {
        ...
        try {
            ...
            foreach ($paginated->items() as $report) {
                ...
                $item['event'] = $eventForDetails ? array(
                ...
                $item['host_user'] = $hostUserForDetails ? array(
    ...
    public function updateReportStatus(Request $request)
    {
        ...
        try {
            ...
            $report->update(array('status' => $data['status']));
    ...
    public function getSuperAdminEventsList(Request $request)
    {
        ...
        $result = array(); // Holds response data
        ...
        if (!empty($data['date_from']) && !empty($data['date_to']) && $data['date_from'] > $data['date_to']) {
            $result = array(
                ...
                'error' => array(
                    'error_code' => 'E001',
                    'error_message' => array('date_to' => array('The date_to must be after or equal to date_from.')),
                ),
            );
    ...
    {
        // Initialize result container
        $result = array(); // Holds final payload
        ...
        try {
            ...
            $mediaRecords = $eventMediaModel->get_event_media_list(array('event_id' => $event->event_id));
            $mediaData = array(
                'thumbnail' => array(),
                'banner' => array(),
                'flyer' => array(),
            ...
            // Social media
            $socialMediaRecords = $eventSocialMediaModel->get_event_social_media_list(array('event_id' => $event->event_id));
            ...
            // Tickets
            $ticketsRecords = $ticketModel->get_tickets_list(array('event_id' => $event->event_id));
            $ticketsData = array();
            foreach ($ticketsRecords as $ticket) {
                $category = $ticketCategoryModel->get_ticket_category(array('ticket_category_id' => $ticket->ticket_category_id));
                $ticketsData[] = array(
            ...
            // Venue
            $venue = $venueModel->get_venue(array('event_id' => $event->event_id));
            $venueData = null;
            if (!empty($venue)) {
                $country = !empty($venue->country_id) ? $countryModel->get_country(array('country_id' => $venue->country_id)) : null;
                $venueData = array(
            ...
            // Artists
            $artistsRecords = $eventArtistModel->get_event_artists_list(array('event_id' => $event->event_id));
            $artistsData = array();
            foreach ($artistsRecords as $artist) {
                $artistSocials = $artistSocialMediaModel->get_artist_social_media_list(array('event_artist_id' => $artist->event_artist_id));
                $artistSocialData = array();
            ...
            // Terms
            $terms = $eventTermsConditionModel->get_event_terms_condition(array('event_id' => $event->event_id));
            $termsData = !empty($terms) ? array(
                'event_terms_id' => $terms->event_terms_id,
                'terms_content' => $terms->terms_content,
            ) : null;

            // Coupons
            $couponsRecords = $couponModel->get_coupons_list(array('event_id' => $event->event_id));
            $couponsData = array();
            ...
            // Host info
            $hostUser = $hostUserModel->get_host_user(array('host_user_id' => $event->host_user_id));
            $businessInfo = null;
            if (!empty($hostUser) && !empty($hostUser->business_id)) {
                $business = $businessModel->get_business(array('business_id' => $hostUser->business_id));
                if (!empty($business)) {
                    $businessInfo = array(
    ...
    {
        // Initialize result container
        $result = array(); // Holds final response payload
    ...
    public function getSuperAdminAttendeesList(Request $request)
    {
        ...
        $result = array(); // Stores success or error payload
        ...
        try {
            ...
            $filters = array(); // Collects optional filters
            ...
            $eventsByUserId = array(); // Initialize grouping array
            foreach ($eventsData as $eventRow) {
                $userId = $eventRow->user_id; // Current attendee ID
                if (!isset($eventsByUserId[$userId])) {
                    $eventsByUserId[$userId] = array(); // Initialize user entry if missing
                }
            ...
            $attendeesArray = array(); // Holds formatted attendees
            foreach ($attendees as $attendee) {
                ...
                $eventsList = isset($eventsByUserId[$userId]) ? $eventsByUserId[$userId] : array(); // Retrieve events list if available
    ...
    public function toggleEventHostBlockStatus(Request $request)
    {
        ...
        $result = array(); // Will be populated before returning
