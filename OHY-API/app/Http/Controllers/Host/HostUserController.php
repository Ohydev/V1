            ...
            try {
                ...
                try {
                    ...
                    if ($existingHostUser && ! $existingHostUser->is_email_verification_complete) {
                        ...
                        $updateCondition = ['host_user_id' => $existingHostUser->host_user_id];
                ...
                if (! empty($hostUser->business_id) && $hostUser->business_id != null) {
                    ...
                    if (! empty($business) && $business != null) {
                        ...
                        if (! empty($business->businessIntersection)) {
                            $userInfo['business']['business_intersection'] = [
        ...
        if (! $validation->fails()) {
            try {
                ...
                if (! empty($data['state']) && ! empty($data['zipcode'])) {
                    ...
                    if ($response->failed()) {
                        Log::warning('Zippopotam returned non-success status in updateHostUserProfile', [
                            'zipcode' => $zipcode,
                ...
                try {
                    ...
                    $userLookupCondition = ['email' => $hostEmail];
                    ...
                    if (! empty($currentHostUser->business_id) && $currentHostUser->business_id != null) {
                        ...
                        if ($updateResult > 0) {
                            ...
                            if (! empty($updatedBusiness->businessIntersection)) {
                                $businessInfo['business_intersection'] = [
        ...
        if (! $validation->fails()) {
            try {
                ...
                if ($hostUser) {
                    ...
                    $queryCondition = ['host_user_id' => $hostUser->host_user_id];
        ...
        if (! $validation->fails()) {
            try {
                ...
                try {
                    ...
                    // If an end user exists with the same email, sync the hashed password
                    $userLookupCondition = ['email' => $hostUser->email];
                    if ($userModel->check_user_exists($userLookupCondition)) {
                        $userModel->update_user_data($userLookupCondition, [
                            'password' => $hashedPassword,
                        ]);
                    }
                ...
                if (! $existingHostUser || ($existingHostUser && ! $existingHostUser->is_email_verification_complete)) {
                    ...
                    try {
                        if ($existingHostUser && ! $existingHostUser->is_email_verification_complete) {
                            ...
                            $updateCondition = ['host_user_id' => $existingHostUser->host_user_id];
                            $hostUserModel->update_host_user_data($updateCondition, [
