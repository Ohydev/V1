...
class EventController extends Controller
{
    ...
    public function getEventCreationMasterData(Request $request)
    {
        ...
        try {
            ...
            $eventCategoriesQueryCondition = array(); // Empty array to get all categories
            ...
            $socialMediaPlatformsArray = array(
                array(
    ...
    public function saveEventStep1(Request $request)
    {
        ...
        if ($request->hasFile('event_video')) {
            ...
            foreach ($videoFiles as $index => $videoFile) {
                ...
                if (!$videoFile->isValid()) {
                    ...
                    $result = array(
                        ...
                        'error' => array(
                            'error_code' => 'E001',
                            'error_message' => array(
                                'event_video.' . $index => array($uploadErrorMessage)
                            )
                        )
                    );
                ...
                Log::info('Video Upload Diagnostics - File ' . $index, array(
            ...
            try {
                ...
                try {
                    ...
                    if ($isUpdate) {
                        ...
                        $queryCondition = array('event_id' => $eventId); // Find by event ID
                        $updateResult = $eventModel->update_event_data($queryCondition, $eventData);
                        
                        // Get updated event record for response
                        $event = $eventModel->get_event(array('event_id' => $eventId));
                    } else {
                    ...
                    foreach ($socialMediaPlatforms as $requestField => $platformValue) {
                        ...
                        if (!empty($url)) {
                            ...
                            if (!empty($existingSocialMedia)) {
                                ...
                                $eventSocialMediaModel->update_event_social_media_data($socialMediaQuery, array('url' => $url));
                            } else {
                                // Create new record
    ...
    public function getTicketCategories(Request $request)
    {
        ...
        if (!$validation->fails()) {
            try {
                ...
                $categoriesQueryCondition = array(
        ...
        if (!$validation->fails()) {
            try {
                ...
                try {
                    ...
                    $existingTicketsQueryCondition = array(
                        'event_id' => $data['event_id'], // Filter by event ID
                    );
        ...
        if (!$validation->fails()) {
            try {
                ...
                try {
                    ...
                    $existingArtistsQueryCondition = array(
                        'event_id' => $data['event_id'], // Filter by event ID
                    );
        ...
        if (!$validation->fails()) {
            try {
                ...
                try {
                    ...
                    $existingCouponsQueryCondition = array(
                        'event_id' => $data['event_id'], // Filter by event ID
                    );
                    ...
                    if (isset($data['coupons']) && is_array($data['coupons']) && count($data['coupons']) > 0) {
                        ...
                        foreach ($data['coupons'] as $couponData) {
                            ...
                            $couponCodeQueryCondition = array(
    ...
    public function getEventSummary(Request $request)
    {
        ...
        if (!$validation->fails()) {
            try {
                ...
                // Group media by type
                $eventMedia = array(
                    'thumbnail' => array(), // Thumbnail media array
                    'banner' => array(), // Banner media array
                    'flyer' => array(), // Flyer media array
                    'video' => array(), // Video media array
                );
                ...
                $socialMediaQueryCondition = array(
                    'event_id' => $data['event_id'], // Filter by event ID
                );
                ...
                foreach ($artists as $artist) {
                    ...
                    $artistSocialMediaQueryCondition = array(
    ...
    public function getAttendeesList(Request $request)
    {
        ...
        if (!$validation->fails()) {
            try {
                ...
                $filters = array(); // Initialize filters array
                ...
                $eventsByUserId = array(); // Initialize array to store events grouped by user
                foreach ($eventsData as $eventData) {
                    $userId = $eventData->user_id; // Get user ID
                    
                    // Initialize array for this user if not exists
                    if (!isset($eventsByUserId[$userId])) {
                        $eventsByUserId[$userId] = array(); // Initialize empty array
                    }
                ...
                foreach ($attendees as $attendee) {
                    ...
                    $eventsList = isset($eventsByUserId[$userId]) ? $eventsByUserId[$userId] : array();
